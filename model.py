...
class ResidualBlockUpsample(nn.Module):
    ...
    def __init__(self, in_ch: int, out_ch: int):
        super().__init__()
        self.subpel_conv = nn.ConvTranspose2d(
            in_ch,
            out_ch,
            kernel_size=3,
            stride=2,
...
class MAE_Encoder(torch.nn.Module):
    def __init__(
        ...
        self.patchify = torch.nn.Sequential(
            ResidualBlockWithStride(3, emb_dim // 4),
            ResidualBlockWithStride(emb_dim // 4, emb_dim // 2),
            ResidualBlockWithStride(emb_dim // 2, emb_dim),
        )
...
class MAE_Decoder(torch.nn.Module):
    def __init__(
        ...
        self.patch2img = Rearrange(
            # (16 * 16, 1, 3 * 8 * 8) to (1, 3, 16*8, 16*8)
            "(h w) b (c p1 p2) -> b c (h p1) (w p2)",
            p1=patch_size,
            p2=patch_size,
            h=image_size // patch_size,
        )
        self.patch2feature = Rearrange(
            # (16 * 16, 1, 3 * 8 * 8) to (1, 3*8*8, 16,16)
            "(h w) b (c p1 p2) -> b (c p1 p2) h w",
            p1=patch_size,
            p2=patch_size,
            h=image_size // patch_size,
        )
        self.patch_upsample = torch.nn.Sequential(
            ResidualBlockUpsample(emb_dim, emb_dim // 2),
            ResidualBlockUpsample(emb_dim // 2, emb_dim // 4),
            ResidualBlockUpsample(emb_dim // 4, 3)
        )
        self.init_weight()

    def synthesis_img(self, img):
        img = self.patch2feature(img)
        img = self.patch_upsample(img)
        return img
    ...
    def forward(self, features, backward_indexes):
        ...
        img = self.synthesis_img(patches)
